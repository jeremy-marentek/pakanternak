...
{
    $origin = $this->db->get_where('alamat', ['id_pengguna' => $this->session->userdata('id'), 'set_alamat_utama' => 1, 'tipe' => 1])->row();
    $destination = $this->db->get_where('alamat', ['id_pengguna' => $this->session->userdata('id'), 'set_alamat_utama' => 1, 'tipe' => 2])->row();
...
public function index()
{
	...
	foreach ($list_penjual as $id_penjual => $detail_list) {
		...
		foreach ($detail_list as $detail) {
			// Ambil respons ongkir untuk penjual yang sesuai dengan produk ini
			$ongkir_data = $data['ongkir'][$detail['id_penjual']];
		
			?>
			 
			<!-- Tambahkan HTML di sini untuk menampilkan biaya pengiriman untuk setiap produk -->
			<div>
